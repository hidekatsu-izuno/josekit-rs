            ...
            b'-' => {
                ...
                match alg.as_str() {
                    ...
                    "ED25519 PRIVATE KEY" => {
                        let key_info = Self::detect_from_der(&data)?;
                        if key_info.is_public_key() || 
                            !matches!(key_info.alg(), Some(KeyAlg::Ed {
                    ...
                    "X25519 PRIVATE KEY" => {
                        let key_info = Self::detect_from_der(&data)?;
                        if key_info.is_public_key() || 
                            !matches!(key_info.alg(), Some(KeyAlg::Ecx {
        ...
        let key_info = match reader.next().ok()? {
            ...
            Some(DerType::Integer) => match reader.next().ok()? {
                ...
                Some(DerType::OctetString) => {
                    let curve = match reader.next().ok()? {
                        Some(DerType::Other(DerClass::ContextSpecific, 0)) => match reader.next().ok()? {
                            Some(DerType::ObjectIdentifier) => match reader.to_object_identifier().ok()? {
                                val if val == *OID_PRIME256V1 => Some(EcCurve::P256),
                                val if val == *OID_SECP384R1 => Some(EcCurve::P384),
                                val if val == *OID_SECP521R1 => Some(EcCurve::P521),
...
mod tests {
    ...
    fn test_detect_ec_der_private_key() -> Result<()> {
        for curve in &[EcCurve::P256, EcCurve::P384, EcCurve::P521, EcCurve::Secp256k1] {
            let input = load_file(
                &format!("der/EC_{}_pkcs8_private.der", curve.name())
            )?;
            ...
            assert_eq!(key_info.alg(), Some(KeyAlg::Ec {
                curve: Some(*curve)
            }));
            assert_eq!(key_info.is_public_key(), false);
        }
    ...
    fn test_detect_ec_der_public_key() -> Result<()> {
        for curve in &[EcCurve::P256, EcCurve::P384, EcCurve::P521, EcCurve::Secp256k1] {
            let input = load_file(
                &format!("der/EC_{}_spki_public.der", curve.name())
            )?;
    ...
    fn test_detect_ec_raw_private_key() -> Result<()> {
        for curve in &[EcCurve::P256, EcCurve::P384, EcCurve::P521, EcCurve::Secp256k1] {
            let input = load_file(
                &format!("der/EC_{}_raw_private.der", curve.name())
            )?;
    ...
    fn test_detect_ec_pem_private_key() -> Result<()> {
        for curve in &[EcCurve::P256, EcCurve::P384, EcCurve::P521, EcCurve::Secp256k1] {
            let input = load_file(
                &format!("pem/EC_{}_private.pem", curve.name())
            )?;
    ...
    fn test_detect_ec_pem_public_key() -> Result<()> {
        for curve in &[EcCurve::P256, EcCurve::P384, EcCurve::P521, EcCurve::Secp256k1] {
            let input = load_file(
                &format!("pem/EC_{}_public.pem", curve.name())
            )?;
    ...
    fn test_detect_ec_traditional_pem_private_key() -> Result<()> {
        for curve in &[EcCurve::P256, EcCurve::P384, EcCurve::P521, EcCurve::Secp256k1] {
            let input = load_file(
                &format!("pem/EC_{}_traditional_private.pem", curve.name())
            )?;
    ...
    fn test_detect_ec_jwk_private_key() -> Result<()> {
        for curve in &[EcCurve::P256, EcCurve::P384, EcCurve::P521, EcCurve::Secp256k1] {
            let input = load_file(
                &format!("jwk/EC_{}_private.jwk", curve.name())
            )?;
    ...
    fn test_detect_ec_jwk_public_key() -> Result<()> {
        for curve in &[EcCurve::P256, EcCurve::P384, EcCurve::P521, EcCurve::Secp256k1] {
            let input = load_file(
                &format!("jwk/EC_{}_public.jwk", curve.name())
            )?;
    ...
    fn test_detect_ed_der_private_key() -> Result<()> {
        for curve in &[EdCurve::Ed25519, EdCurve::Ed448] {
            let input = load_file(
                &format!("der/{}_pkcs8_private.der", curve.name().to_uppercase())
            )?;
            ...
            assert_eq!(key_info.alg(), Some(KeyAlg::Ed {
                curve: Some(*curve)
            }));
            assert_eq!(key_info.is_public_key(), false);
        }
    ...
    fn test_detect_ed_der_public_key() -> Result<()> {
        for curve in &[EdCurve::Ed25519, EdCurve::Ed448] {
            let input = load_file(
                &format!("der/{}_spki_public.der", curve.name().to_uppercase())
            )?;
    ...
    fn test_detect_ed_pem_private_key() -> Result<()> {
        for curve in &[EdCurve::Ed25519, EdCurve::Ed448] {
            let input = load_file(
                &format!("pem/{}_private.pem", curve.name().to_uppercase())
            )?;
    ...
    fn test_detect_ed_pem_public_key() -> Result<()> {
        for curve in &[EdCurve::Ed25519, EdCurve::Ed448] {
            let input = load_file(
                &format!("pem/{}_public.pem", curve.name().to_uppercase())
            )?;
    ...
    fn test_detect_ed_traditional_pem_private_key() -> Result<()> {
        for curve in &[EdCurve::Ed25519, EdCurve::Ed448] {
            let input = load_file(
                &format!("pem/{}_traditional_private.pem", curve.name().to_uppercase())
            )?;
    ...
    fn test_detect_ed_jwk_private_key() -> Result<()> {
        for curve in &[EdCurve::Ed25519, EdCurve::Ed448] {
            let input = load_file(
                &format!("jwk/OKP_{}_private.jwk", curve.name())
            )?;
    ...
    fn test_detect_ed_jwk_public_key() -> Result<()> {
        for curve in &[EdCurve::Ed25519, EdCurve::Ed448] {
            let input = load_file(
                &format!("jwk/OKP_{}_public.jwk", curve.name())
            )?;
    ...
    fn test_detect_ecx_der_private_key() -> Result<()> {
        for curve in &[EcxCurve::X25519, EcxCurve::X448] {
            ...
            assert_eq!(key_info.alg(), Some(KeyAlg::Ecx {
                curve: Some(*curve)
            }));
            assert_eq!(key_info.is_public_key(), false);
        }
